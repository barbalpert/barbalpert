...
| 8 | 🔴 **Deploy the form handler — forms do not work until this is done.** Both forms POST to a Google Apps Script running in Barb's own Google account (free, no third-party service). Follow **`form-handler/README.md`** — about 10 minutes — then replace `PASTE_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE` in `contact.html` and `home-value.html` with the Web app URL. | `form-handler/`, `contact.html`, `home-value.html` |
